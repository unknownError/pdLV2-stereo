...
cat $1 | grep "\$0-control" | sed s/"^.*\$0-"/""/ | sed s/";$"/""/              | sed s/"control\([0-9]\)"/"\1"/ |  sed s/"^\([0-9]\) label\:\([^ ]*\)"/"Name\1 \2 \n\1"/ | sed s/"^\([0-9]\) range\: \([0-9.]*\) \([0-9.]*\) \([0-9.]*\)"/"control\1minimum \2 \ncontrol\1default \3 \ncontrol\1maximum \4"/

grep -o "pluginName: [a-zA-Z0-9]*" $1 | sed "s/:/""/" 
...
#cat controls.txt | sed s/"^\([0-9]\) range\: \([0-9.]*\) \([0-9.]*\) \([0-9.]*\)"/"control\1minimum \2 \ncontrol\1default \3 \ncontrol\1maximum \4"/ | sed s/"\([^ ]*\) \(.*\)"/"sed s\/\"\1\"\/\"\2\"\/ | \\\\"/ >> replaceTemplate.sh 
